...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations) -> None:
        ...
        if self.opt.progressive_train:#no
        ...
        gs_fea_dim = self.deform.deform.node_num if args.skinning and self.deform.name == 'node' else self.dataset.hyper_dim#8 hyper不是skinning的512应该 给之后cat用的
        self.gaussians = GaussianModel(dataset.sh_degree, fea_dim=gs_fea_dim, with_motion_mask=self.dataset.gs_with_motion_mask)

        self.scene = Scene(dataset, self.gaussians, load_iteration=-1)#这里的二次初始化一直有问题之前
        self.gaussians.training_setup(opt)#设置一些优化相关的参数
        if self.deform.name == 'node' and not deform_loaded:
            if not self.dataset.is_blender:#随机初始化  causes the control points to be initialized from random point clouds instead of COLMAP point clouds.移除可以用各向同性gaussian初始化点
...
#        dpg.show_viewport()
    ...
    def train(self, iters=5000):
        if iters > 0:
            for i in tqdm.trange(iters):
                if self.deform.name == 'node' and self.iteration_node_rendering < self.opt.iterations_node_rendering:#都是1W为啥
                    self.train_node_rendering_step()
                else:
                    self.train_step()
    ...
    def train_step(self):
        ...
        if not self.viewpoint_stack:#不渐进 skip
        ...
        if self.deform.name == 'mlp' or self.deform.name == 'static':
            ...
            if not self.deform.deform.inited:#初始化过了
            ...
            time_input = self.deform.deform.expand_time(fid)#帧相关 
            N = time_input.shape[0]#512 NO noise 的0.5126  这里进行node对点的插值
            ast_noise = 0 if self.dataset.is_blender else torch.randn(1, 1, device='cuda').expand(N, -1) * time_interval * self.smooth_term(self.iteration)
            ...
            d_xyz, d_rotation, d_scaling, d_opacity, d_color = d_values['d_xyz'], d_values['d_rotation'], d_values['d_scaling'], d_values['d_opacity'], d_values['d_color']
            if self.iteration < self.opt.warm_up:#3k   #(10w,3)
            ...
            elif self.iteration < self.opt.dynamic_color_warm_up:#2w
        ...
        if random_bg_color:#nop
            ...
            gt_alpha_mask = viewpoint_cam.gt_alpha_mask.cuda()#nop2
        ...
        if flow_id2_candidates != [] and lambda_optical > 0 and self.iteration >= self.opt.warm_up:#!nop
        ...
        if not self.opt.no_motion_mask_loss and self.deform.name == 'node' and self.opt.gt_alpha_mask_as_dynamic_mask and viewpoint_cam.gt_alpha_mask is not None and lambda_motion_mask > 0:
            gt_alpha_mask = viewpoint_cam.gt_alpha_mask.cuda()#nop
        ...
        with torch.no_grad():
            ...
            if self.iteration in self.testing_iterations:#every 1k
    ...
    def train_node_rendering_step(self):
        ...
        viewpoint_cam = self.viewpoint_stack.pop(randint(0, len(self.viewpoint_stack) - 1))
        if self.dataset.load2gpu_on_the_fly:#nop
            viewpoint_cam.load2device()
        fid = viewpoint_cam.fid#0.4925 帧处理
        
        time_input = fid.unsqueeze(0).expand(self.deform.deform.as_gaussians.get_xyz.shape[0], -1)#(512, 1)最开始 真的prune很快
        N = time_input.shape[0]

        total_frame = len(self.scene.getTrainCameras())
        time_interval = 1 / total_frame#0.005
        # 0 noise
        ...
        # Loss
        gt_image = viewpoint_cam.original_image.cuda()#(3,400,400)
        if random_bg_color:
            gt_alpha_mask = viewpoint_cam.gt_alpha_mask.cuda()#(4168.4)的总alpha
            gt_image = gt_image * gt_alpha_mask + render_pkg_re['bg_color'][:, None, None] * (1 - gt_alpha_mask)#变成3channel的计算
        Ll1 = l1_loss(image, gt_image)
        loss_img = (1.0 - self.opt.lambda_dssim) * Ll1 + self.opt.lambda_dssim * (1.0 - ssim(image, gt_image))
        loss = loss_img
        ...
        with torch.no_grad():
            if self.iteration_node_rendering < self.opt.iterations_node_sampling:
                ...
                if self.iteration_node_rendering % self.opt.densification_interval == 0 or self.iteration_node_rendering == self.opt.node_warm_up - 1:
                    ...
                    if self.dataset.is_blender:#yes
                ...
                if strategy == 'direct':
                    ...
                    x = original_gaussians.get_xyz.detach()#(35476,3)
                    trans_samp = []
                    ...
                    trans_samp = torch.stack(trans_samp, dim=1)#(3w,16,3) (3w,1,3)->#(3w,48)
                    ...
                    if not self.opt.deform_downsamp_with_dynamic_mask:#不要mask全是1
                        dynamic_mask = torch.ones_like(dynamic_mask)#还是sample了开始初始化的512个index最远的
                    ...
                    gaussians: GaussianModel = self.deform.deform.as_gaussians#[35376][512] 先取后取的关系
                    gaussians._features_dc = torch.nn.Parameter(original_gaussians._features_dc[dynamic_mask][idx])#(512,1,3)
                    gaussians._features_rest = torch.nn.Parameter(original_gaussians._features_rest[dynamic_mask][idx])#(512,0,3) tensor[]
                    gaussians._scaling = torch.nn.Parameter(original_gaussians._scaling[dynamic_mask][idx])#(512,3)
