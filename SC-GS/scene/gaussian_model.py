...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float=5., print_info=True, max_point_num=150_000):#self.cameras_extent#半径5.x传哪去了spatial_lr_scale这里然后又被赋值了
        ...
        allocated_memory_mb = allocated_memory_bytes / 1024 / 1024
        print("已分配的显存量 (MB):", allocated_memory_mb)#(100000,3)这是二次初始化得到的 第一次512 还有第三次？初始化node
        ...
        allocated_memory_bytes = torch.cuda.memory_allocated()
        allocated_memory_mb = allocated_memory_bytes / 1024 / 1024
        print("已分配的显存量 (MB):", allocated_memory_mb)
        ...
        if self.with_motion_mask:#no
            self.feature.data[..., -1] = torch.zeros_like(self.feature[..., -1])

    def training_setup(self, training_args):#传入gaussian 512
        self.percent_dense = training_args.percent_dense#0.01
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")#(512,1)
    ...
    def add_densification_stats(self, viewspace_point_tensor, update_filter):#一个是512,3的值 一个是512的true 得到一个二维度的梯度 norm后成一维度
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter, :2], dim=-1, keepdim=True)
        self.denom[update_filter] += 1#记录这个点被梯度更新了多少次
