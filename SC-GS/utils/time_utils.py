...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, t_multires=6, multires=10,
        ...
        if self.progressive_brand_time:#no
...
class ControlNodeWarp(nn.Module):
    def __init__(self, is_blender, init_pcl=None, node_num=512, K=3, use_hash=False, hash_time=False, enable_densify_prune=False, pred_opacity=False, pred_color=False, with_arap_loss=False, with_node_weight=True, local_frame=False, d_rot_as_res=True, skinning=False, hyper_dim=2, progressive_brand_time=False, max_d_scale=-1, **kwargs):
        ...
        self.hyper_dim = hyper_dim if not skinning else 0  # skinning should not be with hyper 用hyper_dim
        self.is_blender = is_blender
        self.pred_opacity = pred_opacity
        self.pred_color = pred_color
        self.max_d_scale = max_d_scale
        
        self.skinning = skinning  # As skin model, discarding KNN weighting 不用skinning 这个是想怎么实现的
        if with_arap_loss:
        ...
        self.nodes = nn.Parameter(torch.randn(node_num, 3+self.hyper_dim))#512 11
        if not self.skinning:
            self._node_radius = nn.Parameter(torch.randn(node_num))#初始化
            if self.with_node_weight:#yes
                self._node_weight = nn.Parameter(torch.zeros_like(self.nodes[:, :1]), requires_grad=with_node_weight)
        if init_pcl is not None:#第一次没有 直接调用
            self.init(init_pcl)
    ...
    def init(self, opt, init_pcl, hyper_pcl=None, keep_all=False, force_init=False, as_gs_force_with_motion_mask=False, force_gs_keep_all=False, reset_bbox=True, **kwargs):
        ...
        if self.use_hash and reset_bbox:#不用hash train_gui中
            self.network.bbox.data = torch.tensor([init_pcl.min()-.1, init_pcl.max()+.1]).float().cuda()
        self.register_buffer('inited', torch.tensor(True))
        if keep_all or self.node_num > init_pcl.shape[0]:#512>10w不对
            ...
            pcl_to_samp = init_pcl if hyper_pcl is None else hyper_pcl#hyper None
            init_nodes_idx = farthest_point_sample(pcl_to_samp.detach()[None], self.node_num)[0]#512 得到最远pts的idx
        ...
        scene_range = init_pcl.max() - init_pcl.min()#最远点到最近点2.6 所有点 +-1.3之间了感觉
        if self.skinning:#nop
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                ...
                self._node_radius.data = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([self.node_num]).float().to(scene_range.device))#-1.31
                self._node_weight.data = torch.zeros_like(torch.zeros_like(self.nodes[:, :1]))#0
        self.gs = None
        if force_gs_keep_all:#初始点云初始化各向同性
            self.init_gaussians(init_pcl=init_pcl, with_motion_mask=as_gs_force_with_motion_mask)
        else:
            self.init_gaussians(init_pcl=self.nodes[..., :3],with_motion_mask=as_gs_force_with_motion_mask)#只选择部分点来
    ...
    def cal_nn_weight(self, x:torch.Tensor, K=None, feature=None, nodes=None, gs_kernel=True, temperature=1.):
        if self.skinning:
            ...
            if self.cached_nn_weight and self.nn_weight is not None:
                ...
                if self.hyper_dim > 0 and feature is not None:#(10w,3+8)
                    x = torch.cat([x.detach(), feature[..., :self.hyper_dim]], dim=-1)  # cat with hyper coor
                K = self.K if K is None else K
                # Weights of control nodes #(512,11)
    ...
    def node_deform(self, t, detach_node=True, **kwargs):
        ...
        if t.dim() == 3:
            ...
            nodes = self.nodes[..., :3]#(这个是固定的 512,3)取的
        if detach_node:
            nodes = nodes.detach()
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8):
        ...
        # Calculate weights of nodes NN  nodes取前3维度 hyper不要
    ...
    def forward(self, x, t, feature, motion_mask, iteration=0, is_training=True, node_trans_bias=None, node_scaling_bias=None, animation_d_values=None, **kwargs):
        ...
        node_attrs = self.node_deform(t=t, **kwargs)#0.5126
        # Animation skip
        ...
        # Obtain translation
        if self.local_frame:
            local_rot = node_attrs['local_rotation'] + rot_bias#(512,4)
            local_rot_matrix = quaternion_to_matrix(local_rot)#(512,3,3)
            ...
            Ax_avg = (Ax * nn_weight[..., None]).sum(dim=1)#(10w,3,3)->(10w,3) 前面得到的skinning weight操作
            translate = Ax_avg - x#(10w,3)
        else:
            translate = (node_trans[nn_idx] * nn_weight[..., None]).sum(dim=1)
        translate = translate * motion_mask

        ##############################################################################
        if not self.d_rot_as_res: #!skip
    ...
    def init_gaussians(self, init_pcl, with_motion_mask):
        if not hasattr(self, 'gs') or self.gs is None:
            ...
            from scene.gaussian_model import GaussianModel, BasicPointCloud, StandardGaussianModel
            pcd = BasicPointCloud(points=init_pcl.detach(), colors=torch.zeros_like(init_pcl), normals=torch.zeros_like(init_pcl))#512,3
